...
import { CheckSquare, AlertTriangle, Calendar, FolderKanban } from 'lucide-react';

// FIX: Same dueDate handling as TaskBoardPage
function parseDueDate(raw) {
  if (!raw) return null;
  if (Array.isArray(raw)) {
    const [y, mo, d, h = 0, min = 0] = raw;
...
export default function MyTasksPage() {
  ...
  const FilterBtn = ({ s, label }) => (
    <button
      onClick={() => setFilter(s)}
      style={{
        padding: '0.5rem 1rem', borderRadius: '9999px',
        border: '1px solid', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, transition: 'all 0.15s',
    ...
    <div style={{ display: 'flex' }}>
      ...
      <main className="main-content">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
          <FilterBtn s="ALL" label="All" />
          <FilterBtn s="TO_DO" label="To Do" />
          <FilterBtn s="IN_PROGRESS" label="In Progress" />
          <FilterBtn s="DONE" label="Done" />
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {filtered.map(task => {
              ...
              const statusCls = task.status === 'TO_DO' ? 'badge-todo'
                : task.status === 'IN_PROGRESS' ? 'badge-inprogress' : 'badge-done';
                ...
                <div key={task.id} className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                      ...
                      <span className={`badge ${statusCls}`}>{STATUS_LABELS[task.status] || task.status}</span>
                      {isOverdue && (
